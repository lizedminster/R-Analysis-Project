...
#### 1.2 Background
  ...
  This data, the Public Library Survey, is run by the Institute of Museum & Library services. The data sets are publicly accessible & provide operational data on almost 9000 libraries across the US & it's territories. Data is collected annually, and the most recent data set is from 2021. 
...
lib_data_21 <- read.csv("~/GitHub/R-Analysis-Project/data/PLS_FY21_AE_pud21i.csv")

# Read in library data for 2011
setwd("C:/Users/lized/OneDrive/Documents/GitHub/R-Analysis-Project/data")
lib_data_11 <- read.csv("~/GitHub/R-Analysis-Project/data/pupld11b.csv")
```
...
#### 3.2 
We used linear regression to identify traits that would significantly impact one of 3 dependent variables: Number of Visits (VISITS), The total circulation of books (TOTCIR), and The number of registered users (REGBOR)
...
We removed rows where the population, the number of visits, the circulation, or the number of users was missing. This removes 45 rows in 2021, bringing our data from 9215 to 9170.This removes 55 rows in 2011, bringing our data from 9315 to 9260.
...
For each of the dependent variables, we used the formula LOANTO~dependent variable. We chose LOANTO because it is a variable that represents the activity & influence of a library without being directly related to any of the three variables. We removed the same entries as the linear regression 

#### 3.2.1 
For linear regression:
The significant variables for Visits were: EBOOK + PRMATEXP + VIDEO_DL + PHYSCIR + VIDEO_PH. The significant variables for Circulation were: EBOOK + CAPITAL + PRMATEXP + PITUSR + LOANTO + ELMATCIR + PHYSCIR
The significant variables for Users were:EBOOK

The following plots demonstrate the relationship between the fitted values & the residuals for these three models: 
```{r echo = FALSE}
setwd("C:/Users/lized/OneDrive/Documents/GitHub/R-Analysis-Project/data")
...
plot(visitsAdjusted.vs.significant$residuals~visitsAdjusted.vs.significant$fitted.values, xlab = "visits fitted", ylab = "visits residuals")
abline(h=mean(visitsAdjusted.vs.significant$residuals), col='red')
plot(totcirAdjusted.vs.significant$residuals~totcirAdjusted.vs.significant$fitted.values,
xlab = "circulation fitted", ylab = "circulation residuals")
abline(h=mean(totcirAdjusted.vs.significant$residuals), col='blue')
plot(regborAdjusted.vs.significant$residuals~regborAdjusted.vs.significant$fitted.values,
xlab = "users fitted", ylab = "users residuals")
abline(h=mean(regborAdjusted.vs.significant$residuals), col='green')
```
...
The significant variables for Visits were: BKVOL + EBOOK + LOANTO + AUDIO_DL 
The significant variables for Circulation were: BKVOL + EBOOK + PRMATEXP + PITUSR + LOANTO + LOANFM + AUDIO_DL + VIDEO_DL
The significant variables for Users were:LOANFM

The following residual plots for 2011 are below:
```{r echo = FALSE}
setwd("C:/Users/lized/OneDrive/Documents/GitHub/R-Analysis-Project/data")
data <- read.csv("~/GitHub/R-Analysis-Project/data/pupld11b.csv")
#This data specifically is the 2011 data
...
```
####3.2.2
For ANOVA, the now-independent dependent variables have significant differences between the quantile groups for all 3 dependent variables. 
...
```
## 4.0 Conclusions
Going forward, we are planning to compare more of the years in a time-series type of way. We were surprised to see patterns in the residuals of both the 2011 data and the 2021 data. We plan to explore more variables & their effect on the linear models in order to address this. 
We weren't surprised to see that different quantile groups of the dependent variables had different LOANTO means.
