...
First, we began by fitting a model for each of the adjusted dependent variables: Visits, Circulation, & Users using every selected trait. The adjustment was dividing the number of visits/circulation/users by the number of people in the library's given legal service area. Then, for each of the models, we made a second model that included only the significant variables. This is based on the most recent data: 2021. 

We removed rows where the population, the number of visits, the circulation, or the number of users was missing. This removes 45 rows, bringing our data from 9215 to 9170. 
